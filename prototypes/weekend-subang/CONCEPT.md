...
| 단계 | 목표 | 검증 | 상태 |
|---|---|---|---|
| **A** | 윈도우 + 카메라 + 플레이어 도형 + 화살표 이동 + Focus 속도 변화 | 화면 안에서 움직이고 Shift 시 절반 속도 | (in progress) |
| **A2** | 270×480 RT + 540×960 nearest 업스케일 (faux-pixel) | 도형이 픽셀화돼 보임 | — |
| B | Auto-fire 위 방향 + PlayerBullet + 화면 밖 despawn + Focus 시 spread 변화 | 1초당 ~10발, lifetime 동작 | — |
